#!/usr/bin/env python3
"""
X-VLA Concept Ablation using SAE Features on SimplerEnv.

Runs rollout episodes with concept features ablated (zeroed) via
PerTokenAblationHook to measure causal impact on task success in
SimplerEnv (WidowX and Google Robot tasks).

Adapted from xvla_concept_ablation.py (LIBERO) + xvla_simplerenv_eval.py:
...
Usage:
    ...
    # Single layer WidowX ablation
    python experiments/xvla_simplerenv_concept_ablation.py \
        --robot widowx --layer 12 --n-episodes 5 --top-n-features 30
    ...
    # Google Robot
    python experiments/xvla_simplerenv_concept_ablation.py \
        --robot google-robot --all-layers --n-episodes 5

    # Skip baseline
    python experiments/xvla_simplerenv_concept_ablation.py \
        --robot widowx --layer 12 --n-episodes 5 --skip-baseline
"""
    ...
    try:
        for concept_name, concept_info in concepts.items():
            ...
            for task_name in tasks:
                ...
                rate = sum(successes) / len(successes)
                baseline_rate = baseline_results[task_name]["success_rate"]
                delta = rate - baseline_rate
                concept_results["tasks"][task_name] = {
                    "success_rate": rate,
                    "delta": delta,
                    "successes": successes,
                }
...
def main(cfg):
    ...
    print(f"Concept ablation: {cfg.robot} | layers={layers} | "
          f"eps={cfg.n_episodes} | top_n={cfg.top_n_features}")
    ...
    for layer_idx in layers:
        ...
        ablation_results = run_ablation_experiment(
            ...
            cfg.top_n_features, skip_baseline=cfg.skip_baseline,
        )
    ...
    print(f"\nX-VLA SimplerEnv concept ablation complete")
