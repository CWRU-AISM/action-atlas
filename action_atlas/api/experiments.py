...
GROOT_ABLATION_DIR = DATA_ROOT / "groot_rollouts" / "sae_feature_ablation"
GROOT_STEERING_DIR = DATA_ROOT / "groot_rollouts_batch2" / "sae_steering"
GROOT_PROBING_DIR = DATA_ROOT / "groot_rollouts" / "sae_probing"
...
def get_vla_findings():
    ...
    if model == 'pi05':
        findings = {
            ...
            'summary': '20 validated findings from 6,000+ LIBERO episodes. Key discoveries: pathway specialization (PaliGemma=WHAT, Expert=HOW), width-dependent ablation sensitivity in 1024-dim space, and phase-specific steering.',
            'key_findings': [
                ...
                {'id': 'width_dependent_ablation', 'title': 'Width-Dependent Ablation Sensitivity', 'description': "Ablating just 30 features in Pi0.5's 1024-dim hidden space causes severe failure (-60 to -100pp). OpenVLA-OFT (4096-dim) shows 91.6% of ablations with zero effect, so narrower representations concentrate task information into fewer features.", 'evidence': 'Concept ablation experiments comparing Pi0.5 (1024-dim) vs OpenVLA-OFT (4096-dim).', 'confidence': 'high', 'category': 'causal'},
        ...
        findings = {
            ...
            'key_findings': [
                {'id': 'grid_spatial', 'title': 'Spatially Structured Vision Dependence', 'description': 'Grid ablation (4x4 masking) reveals that specific image regions are load-bearing for task success. For TransferCube, the workspace center is required; for Insertion, the peg region is.', 'evidence': '960 trajectories across 2 tasks x 16 grid cells x 20 episodes.', 'confidence': 'high', 'category': 'vision'},
        ...
        findings = {
            ...
            'key_findings': [
                ...
                {'id': 'all_layers_critical', 'title': 'Every Layer Required (Narrow Architecture)', 'description': "Zeroing any single one of X-VLA's 24 TransformerBlocks causes complete task failure (0% success, baseline 96.7%). At 1024-dim hidden space (same as Pi0.5), X-VLA still shows an 82.3% zero-effect rate under concept ablation, closer to 4096-dim OFT than to Pi0.5.", 'evidence': 'Per-layer grid ablation across 4 LIBERO suites + concept ablation (2,480 pairs).', 'confidence': 'high', 'category': 'architecture'},
        ...
        findings = {
            ...
            'key_findings': [
                ...
                {'id': 'vlm_fragile_l0', 'title': 'VLM L0 Ablation is Severely Disruptive', 'description': 'VLM pathway L0 ablation causes -56.4pp mean delta with 64.3% destructive rate. Later VLM layers show mixed effects. Kill-switches: cat/object (-78.5pp), cat/motion (-73.5pp), object/orange_juice (-70pp).', 'evidence': '210 VLM concept ablation pairs.', 'confidence': 'high', 'category': 'causal'},
        ...
        findings = {
            ...
            'key_findings': [
                ...
                {'id': 'all_dit_critical', 'title': 'Every DiT Layer Required', 'description': "Zeroing any single DiT layer causes complete task failure (0% success, baselines 70-100%). This mirrors X-VLA's narrow architecture finding. GR00T's DiT pathway stays non-redundant even at 2048-dim hidden space.", 'evidence': 'Per-layer grid ablation across 3 LIBERO suites (Goal, Object, Long).', 'confidence': 'high', 'category': 'causal'},
            ...
            'limitations': ['Only LIBERO benchmark (3 suites: Goal, Object, Long)', 'DiT grid ablation shows uniform 0% (all layers required, no gradient)'],
        }
    else:
